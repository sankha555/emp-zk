...
class Conv2D : public Layer<T> {
    ...
    void handle_padding(){
        ...
        int padded_image_h = this->image_h + this->pad_h + this->pad_h;
        int padded_image_w = this->image_w + this->pad_w + this->pad_w;

        this->ghost_predecessors.clear();

        T* padded_input = new T[this->in_channels * padded_image_h * padded_image_w];
        int pixels = 0;
        for(int z = 0; z < this->in_channels; z++){
            // if(this->party == ALICE) cout << "\nChannel " << z << ":\n";
            for(int y = 0; y < padded_image_h; y++){
                for(int x = 0; x < padded_image_w; x++){
                    if(y < this->pad_h || y >= this->pad_h + this->image_h){
                        padded_input[
                            z * padded_image_h * padded_image_w +
                            y * padded_image_w +
                            x
                        ] = constant<T>(0);

                        this->ghost_predecessors.insert(z * padded_image_h * padded_image_w +
                            y * padded_image_w +
                            x);

                    } else if(x < this->pad_w || x >= this->pad_w + this->image_w){
                        padded_input[
                            z * padded_image_h * padded_image_w +
                            y * padded_image_w +
                            x
                        ...
                        padded_input[
                            ...
                            z * this->image_h * this->image_w +
                            (y - this->pad_h) * this->image_w +
                            (x - this->pad_w)
                        ];
                    ...
                    //     if(this->party == ALICE) cout << format_EMP_IntFp(
                    //         padded_input[
                    //             z * this->in_channels +
                    //             y * padded_image_w +
                    //             x
        ...
        assert(this->ghost_predecessors.size() == this->in_channels * padded_image_h * padded_image_w - this->input_size);
        for(int e : this->ghost_predecessors){
            cout << e << " ";
        }
        cout << "\n";
        ...
        this->image_h = padded_image_h;
        this->image_w = padded_image_w;
        this->input_size = this->in_channels * this->image_h * this->image_w;

        delete this->input;
        this->input = padded_input;

        this->lower_bounds = new T[this->output_size];
        this->upper_bounds = new T[this->output_size];
        
        this->max_coeffs = this->kernel->params_per_out_channel + 1;  // +1 for bias        
        this->pred_neuron_ids = new int[this->output_size * this->max_coeffs];
        ...
        if(this->party == ALICE) cerr << "completed padding\n";
    }
        ...
        for(int z = 0; z < this->out_channels; z++){
            for(int y = 0; y < this->out_h; y++){
                for(int x = 0; x < this->out_w; x++){
                    int nid = z * this->out_h * this->out_w + 
                              y * this->out_w +
                              x;
