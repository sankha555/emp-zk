...
void backsubstitute_uc_using_prev_layers(Layer<T>* current_layer, Layer<T>* prev_activation, Layer<T>* prev_affine, Layer<T>* input_layer, set<int> neuron_list = {}){
    ...
    if constexpr (std::is_same<IntFp, T>::value && SECURE){
        ...
        T* coeff_sign = new T[2 * prev_affine->output_size];
        T* prev_layer_term = new T[2 * prev_affine->output_size];
        T* constant_terms = new T[neuron_list.size()];  // activation - affine
        ...
        for(int i : neuron_list){
            ...
            // =========== NON-CONSTANT TERMS =========== 
            for(int j = 0; j < prev_affine->output_size; j++){
                prev_layer_term[j]                            = coeff_sign[j]                            * prev_activation->upper_constraints[j * 2 + 0];
                prev_layer_term[j + prev_affine->output_size] = coeff_sign[j + prev_affine->output_size] * prev_activation->lower_constraints[j * 2 + 0];

                new_backsubstituted_upper_constraints[n * (prev_affine->output_size + 1) + j] = current_layer->upper_constraints[i * current_layer->max_coeffs + j] * 
                                                                                                (prev_layer_term[j] + prev_layer_term[j + prev_affine->output_size]);
            }
            
            ZKgeneralTruncAny(
                current_layer->party, 
                new_backsubstituted_upper_constraints + n * (prev_affine->output_size + 1), 
                new_backsubstituted_upper_constraints + n * (prev_affine->output_size + 1), 
                (prev_affine->output_size), 
                FXPSCALE
            );
        ...
        n = 0;
        for(int i : neuron_list){
            new_backsubstituted_upper_constraints[(n + 1)* (prev_affine->output_size + 1) - 1] = constant_terms[n] + 
                                                                                                current_layer->upper_constraints[(i + 1) * current_layer->max_coeffs - 1];
        ...
        n = 0;
        for(int i : neuron_list){
            T* current_lower_constraints = new_backsubstituted_upper_constraints + (n* (prev_affine->output_size + 1));
            ...
            for(int j = 0; j < prev_affine->output_size; j++){
                ...
                prev_layer_term[j]                            = coeff_sign[j]                            * prev_affine->upper_bounds[j];
                prev_layer_term[j + prev_affine->output_size] = coeff_sign[j + prev_affine->output_size] * prev_affine->lower_bounds[j];
            }
            ...
            constant_terms[n] = inner_product_bundle(
                ...
                prev_affine->output_size,
                current_upper_constraints,
                prev_layer_term + prev_affine->output_size,
                current_layer->party
            );
        ...
        n = 0;
        for(int i : neuron_list){
            current_layer->upper_bounds[i] = constant_terms[n] + new_backsubstituted_upper_constraints[(n + 1) * (prev_affine->output_size + 1) - 1];
